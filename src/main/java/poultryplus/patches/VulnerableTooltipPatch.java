...
public class VulnerableTooltipPatch {
    ...
    public static SpireReturn<Void> Insert(VulnerablePower __instance, AbstractCreature owner, String[] DESCRIPTIONS, int amount, @ByRef String[] description){
        ...
        if(AbstractDungeon.player.hasRelic("poultryplus:HookedBeak") && !owner.isPlayer && AbstractDungeon.player.hasPower("Flight")) {
            if(AbstractDungeon.player.hasRelic("Paper Frog")) {
                float MULT = (float) (0.75F + RELICMULT * AbstractDungeon.player.getPower("Flight").amount);
                ...
                float MULT = (float) (0.5F + RELICMULT * AbstractDungeon.player.getPower("Flight").amount);
                int DISPLAY = (int) (100F * MULT);
                if(amount == 1) {
                    description[0] = DESCRIPTIONS[0] + DISPLAY + DESCRIPTIONS[1] + amount + DESCRIPTIONS[2];
                }
