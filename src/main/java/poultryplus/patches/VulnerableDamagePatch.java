...
public class VulnerableDamagePatch {
    ...
    public static SpireReturn<Float> Insert(VulnerablePower __instance, float damage){
        ...
        if(AbstractDungeon.player.hasRelic("poultryplus:HookedBeak") && AbstractDungeon.player.hasPower("Flight")) {
            if(AbstractDungeon.player.hasRelic("Paper Frog")) {
                float DMG = (float) (damage * (1.75F + RELICMULT * AbstractDungeon.player.getPower("Flight").amount));
                return SpireReturn.Return(DMG);
            }
            else {
                float DMG = (float) (damage * (1.5F + RELICMULT * AbstractDungeon.player.getPower("Flight").amount));
                return SpireReturn.Return(DMG);
            }
        }
